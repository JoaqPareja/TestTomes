...
const checkAndAssingImage= ["LIBERATORS","VANQUISHERS","VINDICTORS","CELESTANT-PRIME", "GARDUS STEEL SOUL","KNIGHT-ARCANUM","KNIGHT-DRACONIS","KNIGHT-JUDICATOR", "KNIGHT-QUESTOR","KNIGHT-RELICTOR", "KNIGHT-VEXILLOR", "LORD-AQUILOR","LORD-CELESTANT", "ON DRACOTH", "BASTIAN", "LORD-IMPERATANT", "LORD-RELICTOR", "LORD-VERITANT", "NEAVE'S", "BLACKTALON", "LORAI", "VANDUS HAMMERHAND", "YNDRASTRA", "DRAKESWORN TEMPLAR", "KARAZAI", "KRONDYS", "ON STARDRAKE", "AETHERWINGS", "ANNIHILATORS", "GRYPH-HOUNDS", "WITH METEORIC GRANDHAMMERS", "CONCUSSORS", 
...
const   convertImageToText =async(currentNumber: string)=> {
  ...
  const ret = await worker.recognize("./public/Images/"+currentTome+currentNumber+".jpg");//https://lenguajejs.com/javascript/modulos/dynamic-import/
  await worker.terminate();
  return ret.data.text   
}

const convertToJson =async ()=>{//outputJson
  const firstNumberOfUnits=10;
  const lastNumberOfUnits=70;
  ...
  for(var i= firstNumberOfUnits; i<=lastNumberOfUnits;i++){
    index=i;
    if(i==8||i==9){
    index="0"+i;
    }
